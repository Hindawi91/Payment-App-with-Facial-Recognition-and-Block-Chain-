...
class Blockchain(object):
    def __init__(self):
        ...
        if exists('latest_chain.json'):
            
            with open('latest_chain.json', 'r') as openfile:
                self.chain = json.load(openfile)
        ...
        if not exists('latest_chain.json'):
...
# Create a new block listing key/value pairs of block information in a JSON object. Reset the list of pending transactions & append the newest block to the chain.
    ...
    def new_block(self, proof, previous_hash=None):
        ...
        print(self.chain)
        with open("latest_chain.json", "w") as outfile:
...
# t1 = blockchain.new_transaction("Firas", "Mike", '5 BTC')
...
# # with open("latest_chain.json", "w") as outfile:
# #     outfile.write(blockchain.chain)
